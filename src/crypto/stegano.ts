...
const KEY_LENGTH = 64;
const IV_LENGTH = 32;
//Frame of a Badge in a stringForm
const FRAME_64 =
...
/**
 * @description generates Badge with hidden key and IV
 */
export async function generateBadge({ iv, key }: Creds) {
  const frame = await loadImage(FRAME_64);
...
/**
 * @description reads Badge with hidden key and IV
 */
...
function generateEncryptedImage({ iv, key }: Creds) {
  ...
  //For each cell in the grid, it takes a hex value, converts it to a digit, and then distributes this digit across the 3 color channels (RGB) before storing it.
...
function createColor(altared?: RGB) {
  //It creates a random shade of color or applies a modified version of the cell's color based on the stored value.
  const color = altared ?? [
